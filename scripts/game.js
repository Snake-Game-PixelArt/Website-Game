class Game {
  ...
  endGame() {
    ...
    // const bestGame = lastGames.reduce(
    //   (maxGame, game) => (game.score > maxGame.score ? game : maxGame),
    //   { score: -Infinity }
    // );
    // `${bestGame.score } in ${formatTimeDifference(bestGame.started_at, bestGame.ended_at)}`
...
window.addEventListener("keydown", function (ev) {
  ...
  if (game.snake.directionChanged) {
    if (
      game.snake.directionBuffer[game.snake.directionBuffer.length - 1] !==
      ev.code
    ) {
